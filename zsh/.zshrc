# run startx for me if tty is tty1 promp
...
# start ssh-agent on terminal open
# eval "$(ssh-agent -s)"
...
alias gm='git merge'
alias gchkall='git checkout -- .'

alias sa='eval "$(ssh-agent)"'
alias sg='ssh-add ~/.ssh/github_id_ed25519'
...
tars(){
  if [ "$#" -gt 0 ] && [ "$1" = "clog" ] ; then
    shift
    command "$HOME/Tars/Development/scripts/clogs.sh"
  else
    command echo "use with clog sub-command"
  fi
}
